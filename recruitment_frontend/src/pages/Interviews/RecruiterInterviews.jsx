...
import { FiSearch, FiCalendar, FiClock, FiEdit2, FiEye, FiBriefcase, FiX, FiUser, FiMail, FiPhone, FiFileText, FiGlobe, FiAward, FiVideo, FiRefreshCw, FiCheckCircle, FiXCircle } from 'react-icons/fi';
...
const RecruiterInterviews = () => {
    ...
    const fetchInterviews = async () => {
        try {
            setLoading(true);
            setError(null);
            const data = await interviewService.getMyHRInterviews('', 'All', 0, 1000);
            setInterviews(data.content || []);
        } catch (err) {
            console.error('Error fetching interviews:', err);
            setError(err.response?.data?.message || 'Failed to load HR interviews');
        } finally {
            setLoading(false);
        }
    };
    ...
    const executeReschedule = async () => {
        try {
            ...
            // Re-open the input modal if failed, so user doesn't lose context
            setShowRescheduleModal(true);
        }
    };
    ...
    const initiateSubmitResult = () => {
        ...
        if (selectedResult === 'SHORTLISTED' && selectedInterview?.openingsFilled) {
            showToast('All openings for this job are already filled.', 'error');
            return;
        }

        setShowResultModal(false);

        const isShortlist = selectedResult === 'SHORTLISTED';
        setConfirmationModal({
            isOpen: true,
            title: isShortlist ? 'Confirm Shortlist' : 'Confirm Rejection',
            message: `Are you sure you want to ${isShortlist ? 'shortlist' : 'reject'} ${selectedInterview.candidateName}?${isShortlist ? '\n\nThis candidate will be marked as HR-shortlisted and will be available for offer generation.' : '\n\nThis candidate will be removed from the hiring pipeline.'}`,
    ...
    const executeSubmitResult = async () => {
        try {
            ...
            await interviewService.updateHRResult(selectedInterview.id, selectedResult);

            setConfirmationModal(prev => ({ ...prev, isOpen: false, isLoading: false }));

            setSuccessModal({
                isOpen: true,
                title: selectedResult === 'SHORTLISTED' ? 'Candidate Shortlisted' : 'Candidate Rejected',
                message: `Candidate has been ${selectedResult === 'SHORTLISTED' ? 'shortlisted' : 'rejected'} successfully!`,
                onOk: () => {
                    closeSuccess();
                    setShowResultModal(false);
                    setSelectedInterview(null);
            ...
            // Re-open success/result modal? No, maybe just stay on table
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const getStatusDisplay = (status) => {
        if (status === 'SCHEDULED') {
            return { text: 'Scheduled', className: 'bg-green-100 text-green-800' };
        } else if (status === 'RESCHEDULED') {
            return { text: 'Rescheduled', className: 'bg-green-100 text-green-800' };
        } else if (status === 'COMPLETED') {
            return { text: 'Completed', className: 'bg-gray-100 text-gray-800' };
        } else if (status === 'CANCELLED') {
            return { text: 'Cancelled', className: 'bg-red-100 text-red-800' };
        }
        return { text: status || 'Unknown', className: 'bg-gray-100 text-gray-800' };
    };
    ...
    const getFilteredInterviews = () => {
        return interviews.filter(interview => {
            ...
            // Tab Filter
            const result = interview.result; // Assuming result field exists on interview object
        ...
        <div className="relative min-h-screen bg-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Recruiter Interviews</h1>
                        ...
                        <div className="grid grid-cols-3 gap-2 w-full md:w-auto">
                            <div className="bg-gray-50 border border-gray-200 rounded-xl p-2 sm:p-3 text-center w-full">
                                <span className="block font-bold text-gray-900 text-lg sm:text-xl leading-none mb-1">{totalCount}</span>
                                <span className="block text-gray-500 font-bold text-[9px] sm:text-[10px] uppercase tracking-wider truncate">Total</span>
                            </div>
                            <div className="bg-blue-50 border border-blue-200 rounded-xl p-2 sm:p-3 text-center w-full">
                                <span className="block font-bold text-blue-700 text-lg sm:text-xl leading-none mb-1">{pendingCount}</span>
                                <span className="block text-blue-600 font-bold text-[9px] sm:text-[10px] uppercase tracking-wider flex items-center justify-center gap-1 truncate">
                                    <span className="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0"></span> Pending
                                </span>
                            </div>
                            <div className="bg-green-50 border border-green-200 rounded-xl p-2 sm:p-3 text-center w-full">
                                <span className="block font-bold text-green-700 text-lg sm:text-xl leading-none mb-1">{completedProcessCount}</span>
                                <span className="block text-green-600 font-bold text-[9px] sm:text-[10px] uppercase tracking-wider truncate">Completed</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-3 sm:overflow-x-auto sm:pb-1 no-scrollbar">
                        {['Pending', 'Completed', 'Shortlisted', 'Rejected'].map((filter) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`
                                        group flex items-center justify-center sm:justify-start gap-2 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap border w-full sm:w-auto
                                        ${isActive
                                            ? activeClass
                        ...
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                            ...
                            <div className="space-y-6">
                                {loadingDetails ? (
                                    ...
                                    <>
                                        {/* Header Info / Subtitle - keeping context */}
                                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                <div>
                                                    <h2 className="text-xl font-bold text-gray-900">{candidateDetails.candidateName || selectedInterview.candidateName}</h2>
                                                    <p className="text-gray-500 text-sm mt-1">{candidateDetails.jobTitle || selectedInterview.jobTitle}</p>
                                                </div>
                                                <div className="flex items-center gap-3">
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 text-left">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-base font-semibold text-blue-600">{candidateDetails.candidateEmail || 'N/A'}</p>
                                                    </div>
                                                    <div>
                                                        <label className="block text-sm font-medium text-gray-500 mb-1">PHONE</label>
                                                        <p className="text-base font-semibold text-gray-900">{candidateDetails.candidatePhone || 'N/A'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 text-left">
                                                <h3 className="text-lg font-bold text-gray-900 mb-6 border-b border-gray-100 pb-2">Application Information</h3>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                                    <div>
                                            ...
                                            {(candidateDetails.technicalInterviewResult || candidateDetails.technicalInterviewRemarks) && (
                                                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 text-left">
                                                    <h3 className="text-lg font-bold text-gray-900 mb-6 border-b border-gray-100 pb-2">Technical Interview Feedback</h3>
                                                    {candidateDetails.technicalInterviewResult && (
                                                        <div className="mb-6">
                                                            <label className="block text-sm font-medium text-gray-500 mb-2">
                                                                TECHNICAL RESULT
                                                            </label>
                                                            <span
                                                                className={`inline-flex px-3 py-1 rounded text-sm font-semibold ${candidateDetails.technicalInterviewResult === 'SHORTLISTED'
                                                                    ? 'bg-green-100 text-green-800'
                                                                    : candidateDetails.technicalInterviewResult === 'REJECTED'
                                                    ...
                                                    {candidateDetails.technicalInterviewRemarks && (
                                                        <div>
                                                            <label className="block text-sm font-medium text-gray-500 mb-2">
                        ...
                        <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                            <div className="p-6">
                                ...
                                <div className="mb-6">
                                    ...
                                    {selectedInterview.openingsFilled && (
                                        <p className="mb-3 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                                            All openings for this job are already filled. You can only reject this candidate.
                                        </p>
                                    )}
                                    <div className="space-y-2">
                                        <label className={`flex items-center p-3 border rounded-lg ${selectedInterview.openingsFilled ? 'opacity-60 cursor-not-allowed bg-gray-50' : 'cursor-pointer hover:bg-gray-50'}`}>
                                            <input
                                                type="radio"
                                                name="result"
                                                value="SHORTLISTED"
                                                checked={selectedResult === 'SHORTLISTED'}
                                                onChange={(e) => !selectedInterview.openingsFilled && setSelectedResult(e.target.value)}
                                                disabled={!!selectedInterview.openingsFilled}
                                                className="mr-3 h-4 w-4 text-green-600 focus:ring-green-500"
                                            />
                                            <div>
                                                <span className="font-medium text-green-700">Shortlisted</span>
                                                <p className="text-xs text-gray-500">Candidate passed the HR round</p>
                                            </div>
                                        </label>
